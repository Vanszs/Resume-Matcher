...
def _strip_tags(s: str) -> str:
    return _unescape(_TAG_RE.sub("", s)).strip()
...
def parse_markdown(text: str, source: str) -> list[InternshipItem]:
    """Parse a GitHub markdown internship file into structured records."""
    ...
    for part in parts:
        if re.search(r"<table", part, re.I):
            ...
            for row in _TR_RE.findall(tbody):
                ...
                requires_citizenship = _FLAG_US_CITIZEN in c1  # multi-codepoint emoji
                is_faang = _FLAG_FAANG in raw_role
                requires_adv_degree = _FLAG_ADV_DEGREE in raw_role
                role = _clean_role(raw_role)
                is_closed = _FLAG_CLOSED in c3 or _FLAG_CLOSED in raw_role
            ...
            # ── Non-table block: collect all headings in position order ──
            ...
            for _, text_h in headings:
                if _SKIP_HEADING_RE.search(text_h):
                    continue
                cleaned = _COUNT_SUFFIX_RE.sub("", _ROLE_SUFFIX_RE.sub("", text_h)).strip()
                if cleaned:
