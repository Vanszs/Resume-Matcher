...
async function fetchCoverLetterData(resumeId: string, token?: string): Promise<CoverLetterData> {
...
export default async function PrintCoverLetterPage({ params, searchParams }: PageProps) {
  ...
  const { coverLetter, personalInfo } = await fetchCoverLetterData(resolvedParams.id, resolvedSearchParams?.token);
